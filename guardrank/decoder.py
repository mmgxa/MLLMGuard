...
def load_data_wo_label(data_path):
    ...
    prompt_text = "Dimension: {dimension} Prompt: {prompt} Answer: {response}"
    total_data = []
    n = len(data)
    prompt = data["prompt"]
        ...
        total_data.append(
            {
                "text": prompt_text.format_map(
                    {"dimension": dim, "prompt": prompt[i], "response": response[i]}
                ),
                "category": 0,
            }
        )
...
def evaluate(args, dimension):
    ...
    model = PeftModel.from_pretrained(
        model, args.decoder_model, torch_dtype=torch.float16
    )

    training_args = TrainingArguments(output_dir="tmp", remove_unused_columns=False)

    trainer = Trainer(
        model=model, args=training_args, data_collator=collate, tokenizer=tokenizer
    )
